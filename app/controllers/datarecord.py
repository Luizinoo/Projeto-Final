...
class UserRecord:
    ...
    def add_product(self, name, price, quantity):
        new_product = Product(name, float(price), int(quantity))
    ...
    def update_product_quantity(self, name, new_quantity):
        ...
        if product:
            ...
            self.__write_products()  # Adicionando a escrita do arquivo para refletir a atualização

    # Funções para notícias
    def read_news(self):
        try:
            with open("app/controllers/db/news.json", "r") as arquivo_json:
                news_data = json.load(arquivo_json)
                self.__news = [News(**data) for data in news_data]
        except FileNotFoundError:
            self.__news = []

    def write_news(self):
        try:
            with open("app/controllers/db/news.json", "w") as arquivo_json:
                news_data = [news.to_dict() for news in self.__news]
                json.dump(news_data, arquivo_json)
                print(f'Arquivo de notícias gravado com sucesso!')
        except FileNotFoundError:
            print('O sistema não conseguiu gravar o arquivo de notícias!')

    def add_news(self, title, content, file_path):
        new_news = News(title, content, file_path)
        self.__news.append(new_news)
        self.write_news()

    def get_all_news(self):
        return self.__news

    # Função para salvar produtos do dia
    def save_products_of_the_day(self):
        try:
            with open("app/controllers/db/products_of_the_day.json", "w") as arquivo_json:
                prod_data = [vars(product) for product in self.__products]
                json.dump(prod_data, arquivo_json)
                print(f'Produtos do dia salvos com sucesso!')
        except FileNotFoundError:
            print('O sistema não conseguiu gravar o arquivo dos produtos do dia!')
